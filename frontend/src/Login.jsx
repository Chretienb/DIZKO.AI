...
// Field for the auth card — dark, on the app's actual surface/border tokens
// (the card itself is a soft near-black, not the light exception it started
// as — reported live that white read as too far off the app's dark theme).
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <input id={id} name={id} type={isPw ? (showPass ? 'text' : 'password') : type} value={val}
            ...
            style={{ width:'100%', background:'transparent', border:'none', outline:'none',
              color:'var(--t1)', fontSize:13.5, fontFamily:'inherit', padding:0, caretColor:'var(--brand)' }}/>
        </div>
        {isPw && (
  ...
  return (
    <div style={{ height:'100vh', overflow: isMobile ? 'auto' : 'hidden', background:'var(--bg)',
      ...
      <div style={{ maxWidth:1040, width:'100%', margin:'0 auto', padding: isMobile ? '16px 16px 24px' : '20px 24px',
        ...
        {/* ══ Auth card — soft near-black, matching the reference's two-column
            layout (fields+submit left, social login right, help row at the
            bottom) but on the app's own dark surface tokens instead of a
        ...
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'var(--r-3)', boxShadow:'var(--shadow-1)',
          ...
          {/* Heading */}
          {tab !== 'forgot' && tab !== 'forgot-sent' ? (
            <h2 style={{ margin:0, fontSize:21, fontWeight:800, color:'var(--t1)', letterSpacing:'-.5px',
              ...
              {tab === 'forgot-sent' ? null : (
                <button onClick={() => { setTab('signin'); setFormError('') }}
                  style={{ display:'flex', alignItems:'center', gap:6, background:'none', border:'none',
                    color:'var(--t3)', fontSize:13, cursor:'pointer', marginBottom:14, padding:0 }}>
                  <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round"><polyline points="15,18 9,12 15,6"/></svg>
                  Back
                </button>
              )}
              <h2 style={{ margin:'0 0 6px', fontSize:20, fontWeight:800, color:'var(--t1)', letterSpacing:'-.5px' }}>
                {tab === 'forgot' ? 'Reset password.' : 'Check your inbox.'}
              </h2>
              <p style={{ margin:0, fontSize:13, color:'var(--t3)', lineHeight:1.6 }}>
                {tab === 'forgot'
          ...
          {isInvite && tab === 'signup' && (
            <div style={{ background:'rgba(124,108,240,.12)', border:'1px solid rgba(124,108,240,.3)', borderRadius:10, padding:'10px 14px', marginBottom:14 }}>
              <p style={{ margin:0, fontSize:12.5, color:'var(--brand)', fontWeight:600, lineHeight:1.5 }}>
          ...
          {/* Forgot sent */}
          {tab === 'forgot-sent' ? (
            <div style={{ background:'rgba(34,197,94,.08)', border:'1px solid rgba(34,197,94,.2)', borderRadius:12, padding:'18px' }}>
              <div style={{ fontSize:14, fontWeight:700, color:'#4ade80', marginBottom:6 }}>Link sent ✓</div>
              <div style={{ fontSize:13, color:'rgba(74,222,128,.75)', lineHeight:1.6 }}>
              ...
              <button onClick={() => { setTab('signin'); setFormError('') }}
                style={{ marginTop:14, background:'none', border:'1px solid var(--border)',
                  borderRadius:9, padding:'8px 16px', color:'var(--t2)',
                  fontSize:12, fontWeight:600, cursor:'pointer', width:'100%' }}>
                Back to sign in
              </button>
              <button onClick={() => { setTab('forgot'); setFormError('') }}
                style={{ marginTop:8, background:'none', border:'none', color:'var(--t4)',
              ...
              <div style={{ flex:1, display:'flex', flexDirection:'column', gap:8, minWidth:0 }}>
                ...
                {formError && (
                  <div style={{ display:'flex', alignItems:'center', gap:7, padding:'2px 0' }}>
                    <svg width={13} height={13} viewBox="0 0 24 24" fill="none" stroke="#f87171"
                      strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink:0 }}>
                      <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
                    </svg>
                    <span style={{ fontSize:12.5, color:'#f87171', lineHeight:1.4 }}>{formError}</span>
                  </div>
                )}
              ...
              {tab !== 'forgot' && (
                <>
                  ...
                  <div style={{ flex:1, display:'flex', flexDirection:'column', gap:12, minWidth:0 }}>
                    <p style={{ margin:0, fontSize:12.5, color:'var(--t3)' }}>Or continue with the following options.</p>
          ...
          {/* Footer — just the email icon; a text label next to it would be
              redundant with what the icon itself already says. */}
          <div style={{ marginTop: isMobile ? 20 : 24, paddingTop:16, borderTop:'1px solid var(--border)',
            display:'flex', justifyContent:'flex-end' }}>
            <a href="mailto:team@dizko.ai" aria-label="Email dizko support" title="Any questions? Email us"
              style={{ width:32, height:32, borderRadius:'50%', border:'1px solid var(--border)', flexShrink:0,
                display:'flex', alignItems:'center', justifyContent:'center', color:'var(--t2)', textDecoration:'none' }}>
